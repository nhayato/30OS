...
void init_gdtidt(void) {
  ...
  set_segment(gdt + 1, 0xffffffff, 0x00000000,
              AR_DATA32_RW);  // セグメント番号1番に対する設定
  set_segment(gdt + 2, 0x0007ffff, 0x00280000,
...
void set_segment(struct SEGMENT_DESCRIPTOR *sd, unsigned int limit, int base,
                 int ar) {
  if (limit > 0xfffff) {
    ar |= 0x8000; /* G_bit = 1*/
    limit /= 0x1000;
