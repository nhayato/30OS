...
void HariMain(void)
{
  ...
  // keytable0, keytable1に、バックスペースとEnterのコードを追加
  ...
  int key_shift = 0, key_leds = (binfo->leds >> 4) & 7,
      keycmd_wait = -1;
  int j, x, y, mmx = -1, mmy = -1, mmx2 = 0; // mm = move mode
  int *fat;
  ...
  *((int *)0x0fec) = (int)&fifo;
  ...
  for (;;)
  {
    ...
    else
    {
      ...
      else if (768 <= i && i <= 1023) // コンソール終了状態
      {
