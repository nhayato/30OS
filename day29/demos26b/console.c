  ...
  for (;;)
  {
    ...
    else
    {
      ...
      if (i == 3) //カーソルOFF
      {
        if (cons.sht != 0)
        {
          boxfill8(cons.sht->buf, cons.sht->bxsize, COL8_000000, cons.cur_x, cons.cur_y,
                   cons.cur_x + 7, cons.cur_y + 15);
          cons.cur_c = -1;
        }
      }
      if (i == 4) // コンソールのxボタンクリック
      {
        cmd_exit(&cons, fat);
      }
...
void cmd_start(struct CONSOLE *cons, char *cmdline, int memtotal)
{
  ...
  // コマンドラインに入力さらた文字列を、一文字ずつ新しいコンソールに入力
...
int cmd_app(struct CONSOLE *cons, int *fat, char *cmdline)
{
  ...
  if (finfo != 0)
  {
    ...
    if (appsiz >= 36 && mystrncmp(p + 4, "Hari", 4) == 0 && *p == 0x00)
    {
      segsiz = *((int *)(p + 0x0000));
      esp = *((int *)(p + 0x000c));
      datsiz = *((int *)(p + 0x0010));
      ...
      start_app(0x1b, 0 * 8 + 4, esp, 1 * 8 + 4, &(task->tss.esp0)); // 0x1b (HariMainの番地)
