    ...
    if (fifo32_status(&fifo) == 0) {
      ...
      if (256 <= i && i <= 511) {  // キーボードデータ
        ...
        if (mouse_decode(&mdec, i - 512) != 0) {
          ...
          //   s[2] = 'C';
          // }
          // putfonts8_asc_sht(sht_back, 32, 16, COL8_FFFFFF, COL8_008484, s,
          // 15); マウスカーソルの移動 移動量を加算
          mx += mdec.x;
          my += mdec.y;
...
void console_task(struct SHEET *sheet, unsigned int memtotal) {
  ...
  for (;;) {
    ...
    if (fifo32_status(&task->fifo) == 0) {
      ...
      sheet_refresh(sheet, cursor_x, cursor_x + 8, cursor_x + 8, cursor_y + 16);
    }
  }
}
